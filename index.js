// index.js — CRM2 bot (Расходы + Доходы + Карты)
...
const {
  TELEGRAM_TOKEN,
  SPREADSHEET_ID,                // ID таблицы расходов/доходов
  GOOGLE_SERVICE_ACCOUNT_B64,    // service account JSON в base64
  WEBHOOK_BASE_URL               // (опц.) https://your-domain.com — если есть вебхук
} = process.env;
...
const HELP_TEXT = `Привет! Я бот CRM2.
...
const normalizeDate = raw => {
  ...
  return (!s || isTodayInput(s)) ? new Date() : parseDDMMYYYY(s);
};
const normalizeCurr = raw => String(raw||'').trim().toUpperCase();
const isSkipComment = s =>
  /^(без\s+комментария|пропустить|нет(\s+комментария)?|skip|—|-|–|\.{0,3})$/i
    .test(String(s||'').trim());

async function getTypes(force=false){
  if(!force){ const c=getCache('types'); if(c) return c; }
  const res=await sheets.spreadsheets.values.get({
    spreadsheetId:SPREADSHEET_ID, range:`${SHEET_TYPES}!A2:A`
  });
...
async function getCurrencies(force=false){
  ...
  const res=await sheets.spreadsheets.values.get({
    spreadsheetId:SPREADSHEET_ID, range:`${SHEET_RATES}!A2:A`
  });
...
async function appendExpenseRow(userId,[date,pay,type,geo,amt,curr,comment]){
  ...
  const upd=resp.data.updates?.updatedRange||'';
  const m=upd.match(/!(?:[A-Z]+)(\d+):/);
...
async function undoLastForUser(userId){
  await ensureMetaSheet();
  const res=await sheets.spreadsheets.values.get({
    spreadsheetId:SPREADSHEET_ID, range:`${SHEET_META}!A:D`
  });
  ...
  await sheets.spreadsheets.batchUpdate({
    ...
    requestBody:{ requests:[{ deleteDimension:{ range:{
      sheetId:await getSheetIdByTitle(sheetName),
      dimension:'ROWS', startIndex:rowNumber-1, endIndex:rowNumber
    } } }] }
  });
...
bot.on('text', async (ctx, next)=>{
  ...
  try{
    if(st.mode==='exp'){
      if(st.step==='date'){ st.data.date=normalizeDate(txt); st.step='pay'; setWiz(ctx,st);
        return ctx.reply('Платёжка (AdvCash, Capitalist, Card)', cancelKeyboard()); }
      if(st.step==='pay'){ if(!txt) return ctx.reply('Платёжка не может быть пустой. Введите снова.', cancelKeyboard());
      ...
      if(st.step==='type'){ st.data.type=txt; st.step='geo'; setWiz(ctx,st);
        return ctx.reply('GEO (две буквы, например UA, KZ, PL)', cancelKeyboard()); }
      if(st.step==='geo'){ if(!/^[A-Za-z]{2}$/.test(txt)) return ctx.reply('GEO — две латинские буквы (UA, PL).', cancelKeyboard());
        st.data.geo=txt.toUpperCase(); st.step='amt'; setWiz(ctx,st);
        return ctx.reply('Сумма (число, точка/запятая допустимы)', cancelKeyboard()); }
      if(st.step==='amt'){ const n=Number(txt.replace(',','.')); if(!(n>0)) return ctx.reply('Сумма должна быть > 0. Введите снова.', cancelKeyboard());
      ...
      if(st.step==='curr'){ st.data.curr=normalizeCurr(txt); st.step='comm'; setWiz(ctx,st);
...
Сумма: ${st.data.amt} ${st.data.curr}`, mainKeyboard());
    ...
    if(st.mode==='inc'){
      ...
      if(st.step==='status'){ if(!INCOME_STATUSES.includes(txt)) return ctx.reply('Выберите: Ожидает / Получено / Отклонено.', cancelKeyboard());
        st.data.status=txt; st.step='itype'; setWiz(ctx,st);
        const kb=Markup.keyboard([['Пополнение','Депозит'],['❌ Отмена ввода']]).resize().oneTime();
        return ctx.reply('Тип прибыли:', kb);
      }
      if(st.step==='itype'){ if(!INCOME_TYPES.includes(txt)) return ctx.reply('Выберите: Пополнение / Депозит.', cancelKeyboard());
        st.data.itype=txt; st.step='amt'; setWiz(ctx,st);
        return ctx.reply('Сумма (число):', cancelKeyboard());
      }
      if(st.step==='amt'){ const n=Number(txt.replace(',','.')); if(!(n>0)) return ctx.reply('Сумма должна быть > 0', cancelKeyboard());
        st.data.amt=n; st.step='curr'; setWiz(ctx,st);
        const curr=await getCurrencies();
        const kb=Markup.keyboard([...curr.map(c=>[c]),['❌ Отмена ввода']]).resize().oneTime();
        return ctx.reply('Валюта:', kb);
      }
      if(st.step==='curr'){ st.data.curr=normalizeCurr(txt); st.step='comm'; setWiz(ctx,st);
        return ctx.reply('Комментарий (можно пропустить: «Без комментария»)', commentKeyboard()); }
      if(st.step==='comm'){
